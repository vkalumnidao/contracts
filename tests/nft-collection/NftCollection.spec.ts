...
describe("nft collection smc", () => {
  it("should ignore external messages", async () => {
    ...
    let res = await collection.contract.sendExternalMessage(
      new ExternalMessage({
  ...
  it("should return royalty params", async () => {
    ...
    expect(res.royaltyFactor).toEqual(
      defaultConfig.royaltyParams.royaltyFactor
    );
    expect(res.royaltyAddress.toFriendly()).toEqual(
      defaultConfig.royaltyParams.royaltyAddress.toFriendly()
    );
  });
  ...
  it("should deploy new nft", async () => {
    ...
    expect(initMessage.message.init!.code!.toString()).toEqual(
      defaultConfig.nftItemCode.toString()
    );
    expect(initMessage.message.init!.data!.toString()).toEqual(
      nftItemData.toString()
    );
  });
  ...
  it("should batch deploy nft's", async () => {
    ...
    let [initMessage1, initMessage2] = res.actionList as [
      SendMsgAction,
      SendMsgAction
    ];
    ...
    expect(initMessage1.message.init!.code!.toString()).toEqual(
      defaultConfig.nftItemCode.toString()
    );
    expect(initMessage1.message.init!.data!.toString()).toEqual(
      nftItemData1.toString()
    );
    expect(initMessage2.message.init!.code!.toString()).toEqual(
      defaultConfig.nftItemCode.toString()
    );
    expect(initMessage2.message.init!.data!.toString()).toEqual(
      nftItemData2.toString()
    );
  });
  ...
  it("should send royalty params", async () => {
    ...
    expect(responseMessage.message.info.dest!.toFriendly()).toEqual(
      sender.toFriendly()
    );
    ...
    expect(royaltyAddress.toFriendly()).toEqual(
      defaultConfig.royaltyParams.royaltyAddress.toFriendly()
    );
  });
});
