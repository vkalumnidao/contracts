...
export class NftCollectionLocal {
  ...
  async getCollectionData(): Promise<{
    ...
    let [nextItemId, collectionContent, ownerAddress] = res.result as [
      BN,
      Cell,
      Slice
    ];
  ...
  async getRoyaltyParams(): Promise<RoyaltyParams> {
    ...
    let [royaltyFactor, royaltyBase, royaltyAddress] = res.result as [
      BN,
      BN,
      Slice
    ];
  ...
  async getNftContent(
    ...
    let res = await this.contract.invokeGetMethod("get_nft_content", [
      ...
      {
        type: "cell",
        value: nftIndividualContent.toBoc({ idx: false }).toString("base64"),
      },
    ]);
  ...
  async sendBatchDeployNft(
    from: Address,
    value: BN,
    params: { queryId?: number; items: CollectionMintItemInput[] }
  ) {
