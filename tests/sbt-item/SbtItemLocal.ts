...
export class SbtItemLocal {
  ...
  async getNftData(): Promise<NftDataResponse> {
    ...
    let [initialized, index, collectionAddressSlice, ownerAddress, content] =
      res.result as [BN, BN, Slice, Slice, Cell];
    ...
    return {
      ...
      content: isSingle
        ? decodeOffChainContent(content)
        : content.bits.buffer.toString(),
      contentRaw: content,
    };
  }
  ...
  async sendEditContent(
    from: Address,
    params: { queryId?: number; content: string }
  ) {
